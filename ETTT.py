...
class AI_Bot:
    ...
    def __init__(self):
        ...
        # self.win_pos = []
        # for i in xrange(17):
        #     self.win_pos.append(copy.deepcopy(self.win_possible))
    ...
    def blocks_allowed(self, old_move, block_stat):
        ...
        # self.print_data("final_blocks_allowed", final_blocks_allowed)
    ...
    def cells_allowed(self, temp_board, blocks_allowed, block_stat):
        ...
        for block in blocks_allowed:
            ...
            for i in xrange(start_row, start_row + 4):
                for j in xrange(start_col, start_col + 4):
                    if temp_board[i][j] == '-':
                        cells.append((i,j))
        ...
        if not cells:
            for i in xrange(16):
                ...
                start_row = (i / 4) * 4
                start_col = ((i) % 4) * 4
                for j in xrange(start_row, start_row + 4):
                    for k in xrange(start_col, start_col + 4):
                        if temp_board[j][k] == '-':
                            cells.append((j,k))
        # self.print_data("cells", cells)
        return cells
    ...
    def getBlockStatus(self, block):
    	...
    	if [block[i].count('-') for i in xrange(4)].count(0) == 4:
    		return 'd'
    ...
    def getBlockScore(self, block):
        ...
        # self.print_data("inside getBlockScore and printing getBlockStatus", self.getBlockStatus(block))
    ...
    def lineScore(self, line, curr_block, curr_opp_block, current_block_status):
        ...
        # self.print_data("inside linescore, printing pos", pos)
        # self.print_data("type of pos is ",type(pos))
        # self.print_data("inside linescore, printing neg", neg)
        # self.print_data("type of neg is ",type(neg))
        positive = 1
        negative = 1
        for i in pos:
            positive *= i
    ...
    def getTerminalStatus(self, node, temp_block):
        ...
        for cell in self.centers:
            ...
            # score += score_dict.get(temp_block[cell/4][cell%4], 0)*10
    ...
    def getBoardScore(self, node, temp_block):
        ...
        node_score = [[0]*4 for i in xrange(4)]
        opp_node_score = [[0]*4 for i in xrange(4)]

        for i in xrange(4):
            for j in xrange(4):
                temp = [[node[x][y] for y in xrange(4*j, 4*j + 4)] for x in xrange(4*i, 4*i + 4)]
                node_score[i][j] = self.getBlockScore(temp)
                temp = [[opp_node[x][y] for y in xrange(4*j, 4*j + 4)] for x in xrange(4*i, 4*i + 4)]
                opp_node_score[i][j] = self.getBlockScore(temp)

        # self.print_data("node_score", node_score)
        # self.print_data("opp_node_score", opp_node_score)
        boardScore = []
        for i in xrange(4):
            line = [(i, j) for j in xrange(4)]
            boardScore.append(self.lineScore(line, node_score, opp_node_score, temp_block))
            line = [(j, i) for j in xrange(4)]
            boardScore.append(self.lineScore(line, node_score, opp_node_score, temp_block))
    ...
    def alphabeta(self, node, depth, alpha, beta, maximizingPlayer, old_move, temp_block):
        ...
        # if time.time() - self.came >= self.threshold[self.current_depth-4]:
        ...
        # self.print_data("depth", depth)
        # self.print_data("blocks", blocks)
        # self.print_data("cells_allowed", cells_allowed)
        ...
        if maximizingPlayer:
            ...
            for mov in cells_allowed:
                ...
                # self.print_data("in the depth ", depth)
                # self.print_data("the move selected is ", mov)
                ...
                # self.print_data("the move returned is ", mov)
                # self.print_data("and the value is ", temp)
    ...
    def move(self, current_board, old_move, flag):
        if old_move == (-1, -1):
            return (5, 5)
        # self.print_data("old_move", old_move)
